var jsPsychSurveyFraction = (function (jspsych) {
  "use strict";

  const info = {
    name: "survey-fraction",
    parameters: {
      questions: {
  ...
  class SurveyFractionPlugin {
    ...
    trial(display_element, trial) {
      ...
      if (trial.preamble !== null) {
        html +=
          '<div id="jspsych-survey-fraction-preamble" class="jspsych-survey-fraction-preamble">' +
          trial.preamble +
          "</div>";
      }
      // start form
      if (trial.autocomplete) {
        html += '<form id="jspsych-survey-fraction-form">';
      } else {
        html += '<form id="jspsych-survey-fraction-form" autocomplete="off">';
      }
      ...
      for (var i = 0; i < trial.questions.length; i++) {
        ...
        html +=
          '<div id="jspsych-survey-fraction-' +
          question_index +
          '" class="jspsych-survey-fraction-question" style="margin: 2em 0em;">';
        html +=
          '<p class="jspsych-survey-fraction">' + question.prompt + "</p>";
        ...
        if (question.rows == 1) {
          html +=
            ...
            question_index +
            '"  name="#jspsych-survey-fraction-response-' +
            question_index +
            '" data-name="' +
            question.name +
            '" size="' +
      ...
      // add submit button
      html +=
        '<input type="submit" id="jspsych-survey-fraction-next" class="jspsych-btn jspsych-survey-fraction" value="' +
        trial.button_label +
        '"></input>';
      html += "</form>";
      ...
      display_element
        .querySelector("#jspsych-survey-fraction-form")
        .addEventListener("submit", (e) => {
          ...
          for (var index = 0; index < trial.questions.length; index++) {
            ...
            var q_element = document
              .querySelector("#jspsych-survey-fraction-" + index)
    ...
    simulate_visual(trial, simulation_options, load_callback) {
      ...
      this.jsPsych.pluginAPI.clickTarget(
        display_element.querySelector("#jspsych-survey-fraction-next"),
        data.rt
      );
    }
  }
  SurveyFractionPlugin.info = info;

  return SurveyFractionPlugin;
})(jsPsychModule);
